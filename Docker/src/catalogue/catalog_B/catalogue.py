...
if isLocal and replica_port == '34602':
    replica_host = 'http://catalog-a'
elif isLocal and replica_port == '34612':
    replica_host = 'http://catalog-b'
...
def update(args):
    ...
    if catalog is not None and catalog.quantity > 0:
        ...
        try:
            ...
            print('Exception occurred while writing to replica', file=sys.stdout)
        else:
...
def resync_catalog_db():
    ...
    log_lock.acquire()
    file = open(output_file, "a+")
    file.write('\nGot response: resync catalog db')
    file.close()
    log_lock.release()
    ...
    log_lock.acquire()
    file = open(output_file, "a+")
    file.write('\nReturning from: resync catalog db')
    file.close()
    log_lock.release()
    return '', 200
...
def request_restore_catalog_db():
    log_lock.acquire()
    file = open(output_file, "a+")
    file.write('\nInside: request_restore_catalog_db')
    file.close()
    log_lock.release()
    restore_quantities = {}
    write_lock.acquire()
    ...
    log_lock.acquire()
    file = open(output_file, "a+")
    file.write('\nReturning from: request_restore_catalog_db')
    file.close()
    log_lock.release()
